...
import rarityData from "../assets/data/pppals.json";
...
export default function RarityTablePals() {
  ...
  const handleIdChange = e => {
    ...
    console.log({ id });
    if (id === "") {
      const newPanda = collection.filter(rarityData, x => x.edition === 1);
      setCurrentToken(newPanda[0]);
    } else if (id <= 6363 && id !== "") {
      const newPanda = collection.filter(rarityData, x => x.edition === id);
      setCurrentToken(newPanda[0]);
    }
  };
  ...
  return (
    ...
    <Layout>
      <ImageCard>
        <ImgCardWarp>
          <CardTitleWrap>
            <div>{`${currentToken && currentToken.collection} #${currentToken.edition}`}</div>
            <div>
          ...
          <Image
            ...
            height="500vw"
            alt="PPPandas"
            src={`https://d3e5rpfyahovut.cloudfront.net/public/images/${currentToken.edition}.png`}
          />
          <ButtonWrap>
      ...
      <DataLayout>
        ...
        <GridWrap>
          {currentToken.attributes.map(item => (
            <GridWrapInner key={item.edition}>
              <div>
